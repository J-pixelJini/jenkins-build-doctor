...
class JenkinsClient:
    ...
    def console_log(self, job_name: str, build_numbe"""
Thin wrapper around the Jenkins REST API.
...
class JenkinsClient:
    def __init__(self, config: JenkinsConfig, timeout: int = 15):
        ...
        self.timeout = timeout
        self._auth = (config.user, config.api_token)
        self._crumb_headers: dict | None = None  # lazily fetched, cached per client
    ...
    def _get_crumb_headers(self) -> dict:
        """Most Jenkins instances enable CSRF protection by default, which
        rejects POST requests (like triggering a build) unless they carry a
        crumb token obtained from /crumbIssuer first. This fetches it once
        and caches it; instances with CSRF protection turned off don't expose
        /crumbIssuer, so a 404 there just means no crumb is needed."""
        if self._crumb_headers is not None:
            return self._crumb_headers
        url = f"{self.config.base_url}/crumbIssuer/api/json"
        try:
        ...
        self._crumb_headers = {field: crumb} if crumb else {}
        return self._crumb_headers

    def _post(self, path: str, **kwargs):
        url = f"{self.config.base_url}{path}"
        headers = {**kwargs.pop("headers", {}), **self._get_crumb_headers()}
        resp = requests.post(url, auth=self._auth, timeout=self.timeout, headers=headers, **kwargs)
        resp.raise_for_status()
        return resp
