...
- `build_agent/jenkins_client.py` — thin wrapper over the Jenkins REST API
  (fetch a build's console log, find the last failed build, trigger a new
  build, poll the build queue for the resulting build number). Auth is an API
  token read from environment variables, never hardcoded. Handles Jenkins'
  CSRF crumb requirement automatically: most instances reject POST requests
  (like triggering a build) unless they carry a crumb token from
  `/crumbIssuer` first, so this fetches and caches one, and degrades cleanly
  on instances that have CSRF protection turned off.
- `build_agent/diagnoser.py` — the actual "why did it fail" logic. An ordered
  list of regex signatures (test failure, dependency resolution error, compile
  error, OOM, timeout, permission/auth error), each producing a short summary
  plus the specific log lines that justify it. Falls back to a generic
...
20 tests: 9 cover the diagnosis logic against real sample log fixtures (each
failure category, plus edge cases like an empty log). The other 11 cover
`jenkins_client.py` against mocked HTTP responses — URL construction, auth,
the CSRF crumb flow (including the "no crumb issuer" and "crumb cached
across calls" cases), JSON parsing, 404 handling, and the build-queue
polling logic. Those are mocked because no live Jenkins server was available
to test against directly — they verify the request/response handling is
correct, not that a specific real Jenkins instance behaves identically.
